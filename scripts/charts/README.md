...
| `fetch_enc_features.py` | Rewritten 2026-07-03: 3 bands, cross-band dedupe (rounded-geometry key, harbour > approach > coastal), `bw_cls`/`bw_band` fields, per-class `counts.json`. Dedupe/merge logic unit-tested locally; query pattern re-confirmed live. **Run #1 (2026-07-03) GREEN in 3m17s: 29,851 features deduped, pack = 21,180,023 B (~21 MB).** Coastal counts match S5 baseline exactly (356 wrecks / 217 obstructions / 141 lights / 17 lateral buoys). Exact-position dedupe verified working (safe-water buoys 7+7+7→7); cross-band NEAR-dupes survive where bands digitize the same feature at different generalizations (wreck_point 1,668 raw → 1,357) — candidate fix: LNAM-based identity dedupe. 21 MB vs "single-digit MB" EST — attribute deadweight from `outFields=*` is the likely lever |
