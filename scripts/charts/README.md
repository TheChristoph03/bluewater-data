...
| `fetch_enc_features.py` | Rewritten 2026-07-03: 3 bands, cross-band dedupe (rounded-geometry key, harbour > approach > coastal), `bw_cls`/`bw_band` fields, per-class `counts.json`. Dedupe/merge logic unit-tested locally; query pattern re-confirmed live (HTTP 200 geo+json on real bbox query). Full end-to-end = first `charts-nav-features.yml` run |
